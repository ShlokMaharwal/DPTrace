  ...
  function dp(e, f) {
    if (f === 0 || f === 1) {
      ...
      steps.push({
        ...
        value: f,
        lineNumber: 2,
        explanation: `Base case: dp[${e}][${f}] = ${f}`,
    ...
    if (e === 1) {
      ...
      steps.push({
        ...
        value: f,
        lineNumber: 3,
        explanation: `Base case: dp[1][${f}] = ${f} — with 1 egg must try every floor`,
        tableSnapshot: buildSnapshot(memo, eggs, floors),
      });
      return f;
    }
    ...
    steps.push({
      ...
      value: null,
      lineNumber: 5,
      explanation: `Computing dp[${e}][${f}]: min-worst over all ${f} floor choices`,
      tableSnapshot: buildSnapshot(memo, eggs, floors),
    });
    ...
    steps.push({
      ...
      value: best,
      lineNumber: 10,
      explanation: `dp[${e}][${f}] = ${best} (memoized)`,
      tableSnapshot: buildSnapshot(memo, eggs, floors),
    });
    return best;
  }
...
export const code = `int dp(int e, int f, vector<vector<int>>& memo) {
  ...
  int best = INT_MAX;
  for (int x = 1; x <= f; x++) {
    int worst = 1 + max(dp(e-1,x-1,memo), dp(e,f-x,memo));
    best = min(best, worst);
  }
  return memo[e][f] = best;
}`;

export const lineMap = { base_case: 2, cache_hit: 4, call: 5, cell_fill: 10 };
